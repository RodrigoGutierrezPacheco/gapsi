    ...
    <>
      ...
      >
        <Box
          ...
          onDrop={handleDrop}
        >
          <Typography id="modal-cart-title" variant="h6" component="h2" className="text-center mb-4">
            Carrito de Compras
          </Typography>
          {cartItems.length === 0 ? (
            ...
            <Box>
              {cartItems.map((item, index) => (
                <Box key={index} className="d-flex justify-content-between align-items-center mb-3">
                  <div className="d-flex align-items-center">
                    <img src={item.image} alt={item.name} className="img-fluid rounded me-3" style={{ width: '50px', height: '50px', objectFit: 'cover' }} />
                    <div>
                      <Typography variant="body1" className="mb-0">{item.name}</Typography>
                      <Typography variant="body2" className="text-muted">${item.price}</Typography>
                    </div>
                  </div>
                  <Button onClick={() => removeFromCart(index)} color="error" className="btn btn-danger btn-sm">
                    Eliminar
                  </Button>
                </Box>
              ))}
              <div className="d-flex justify-content-between align-items-center mt-4">
                ...
                <Typography variant="h6">${cartItems.reduce((total, item) => total + item.price, 0)}</Typography>
              </div>
              <Button fullWidth variant="contained" color="primary" className="mt-3">
                Finalizar Compra
              </Button>
            </Box>
          )}
